/**
 * SBTI 中文题库与文案
 * 多语言：可并列增加 sbti-data.en.js 等，导出同名符号或统一 bundle 结构。
 */
...
export const ui = {
  intro: {
    ...
    /** 清除本机测验数据（历史 + 未完成进度） */
    clearAllLocal: '清除所有记录',
    clearAllLocalConfirm: '将删除本机保存的历次结果与未完成进度，且无法恢复。确定清除？',
    /** 首页数据隐私说明（测验数据仅本地） */
    localDataPrivacy: '所有数据完全保存在本设备本地，不会上传到任何服务器。',
  ...
  compute: {
    modeKickerNormal: '你的主类型',
    badgeNormal(similarity, exact) {
      return `匹配度 ${similarity}% · 精准命中 ${exact}/15 维`;
    },
    subNormal: '维度命中度较高，当前结果可视为你的第一人格画像。',
    modeKickerDrunk: '隐藏人格已激活',
    badgeDrunk: '匹配度 100% · 酒精异常因子已接管',
    subDrunk: '乙醇亲和性过强，系统已直接跳过常规人格审判。',
    modeKickerHHHH: '系统强制兜底',
    badgeHHHH(similarity) {
      return `标准人格库最高匹配仅 ${similarity}%`;
    },
